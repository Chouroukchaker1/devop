...
const ResetCodePopup = ({ show, onClose, onCodeSubmit, userId }) => {
  ...
  const handleResetCodeSubmit = async (event) => {
    ...
    try {
      ...
      setTimeout(() => {
        onCodeSubmit(data.tempToken, userId);
        onClose();
      }, 2000);
    } catch (error) {
      console.error('Error:', error);
...
const NewPasswordPopup = ({ show, onClose, tempToken, userId }) => {
...
const SignInPage = ({ onSignInSuccess }) => {
  ...
  const handleEmailSubmit = (userId) => {
    setUserId(userId);
    setShowForgotPassword(false);
    setShowResetCodePopup(true);
  };

  const handleCodeSubmit = (token, userId) => {
    setTempToken(token);
    setUserId(userId);
